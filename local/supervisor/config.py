...
class Settings(BaseSettings):
    ...
    anthropic_auth_token: str = Field(
        default="",
        env="QWEN_GUPSHUP_API_KEY"
    )
    anthropic_model: str = Field(
        default="Qwen3-Coder-480B",
        env="QWEN_SUPERVISOR_MODEL"
    )
    ...
    reports_dir: Path = Field(
        default=Path("local/reports/supervisor"),
        env="REPORTS_DIR"
    )
...
def load_config() -> Settings:
    ...
    import os
    # Lock model to Qwen regardless of shell ANTHROPIC_MODEL
    os.environ["ANTHROPIC_MODEL"] = "Qwen3-Coder-480B"

    # Promote QWEN_GUPSHUP_API_KEY → ANTHROPIC_AUTH_TOKEN so pydantic-settings picks it up
    # (ANTHROPIC_AUTH_TOKEN in shell points to Anthropic, not the Qwen proxy)
    qwen_key = os.environ.get("QWEN_GUPSHUP_API_KEY")
    if qwen_key:
        os.environ["ANTHROPIC_AUTH_TOKEN"] = qwen_key
