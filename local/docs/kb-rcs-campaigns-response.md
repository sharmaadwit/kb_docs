# KB Addition: RCS Campaigns Guide

**For:** Skill code change agent  
**Trace ID:** kb-kb_answer-a4f4b2017d3a4ae2  
**Issue:** Query "RCS campaigns overview" returns IDK despite having RCS and campaign docs separately

---

## The Problem

**Query:** "RCS campaigns overview"  
**Current Response:** "I don't know based on the current docs."  
**Top Source:** `kb/overview/about-projects.md` (confidence: 0.64 — too low)

**Root Cause:** 
- KB has `rcs-overview.md` explaining RCS channel features
- KB has `kb/campaign-manager/*.md` explaining how to create campaigns
- **Missing:** A guide bridging RCS + campaigns together

Users asking "how do I run a campaign using RCS" get IDK because no single doc covers this use case.

---

## The Solution

Create a new file: **`kb/campaign-manager/rcs-campaigns.md`**

This should explain:
1. How to use RCS as a channel in Campaign Manager
2. When to choose RCS campaigns vs other channels
3. Step-by-step setup (RCS agent → templates → campaign creation)
4. Best practices for RCS campaign messaging
5. Analytics specific to RCS campaigns
...
See: [RCS Authentication](../channels/rcs-authentication.md)
...
1. **Create** `kb/campaign-manager/rcs-campaigns.md` with the above content
2. **Update** `kb/campaign-manager/about-campaign-manager.md` to add link: "See also: RCS Campaigns"
3. **Update** `kb/channels/rcs-overview.md` line 90 to link: "[Use RCS as a channel for multi-channel campaigns](../../campaign-manager/rcs-campaigns.md)"
4. **Test** query "RCS campaigns overview" — should now return this new doc with high confidence

---

## Success Criteria

✅ Query "RCS campaigns overview" returns proper answer (not IDK)  
✅ Confidence score improves from 0.64 → 7.0+  
✅ User can find step-by-step guide to create RCS campaign  
✅ Best practices and examples provided

---

**Status:** READY FOR IMPLEMENTATION  
**Priority:** MEDIUM (Bridges existing content gap)  
**Expected Impact:** Fixes 1 IDK query, improves RCS-related search results
