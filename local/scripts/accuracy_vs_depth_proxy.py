...
def fetch_latest_traces(client, name, total):
    ...
    page = 1
    all_traces = []
    while len(all_traces) < total:
        page_limit = min(PAGE_SIZE, total - len(all_traces))
        res = client.api.trace.list(name=name, limit=page_limit, page=page,
...
def main():
    ...
    for t in traces:
        ...
        md = (inp.get("_meta") if isinstance(inp, dict) else None) or (t.metadata if isinstance(t.metadata, dict) else {})
        records.append({
            "trace_id": t.id,
            "timestamp": to_dt(t.timestamp),
            "top_level_session_id": getattr(t, "session_id", None),
