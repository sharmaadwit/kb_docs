...
def main():
    ...
    # Use the Langfuse API directly for fetching observations (v2)
    import requests

    auth = (langfuse_public, langfuse_key)
    url = f"{langfuse_host}/api/public/v2/observations?limit=30"

    response = requests.get(url, auth=auth)
    response.raise_for_status()

    data = response.json()
    raw_obs = data.get('data', [])

    def _obs_to_trace(obs):
        return {
            "id": obs.get("traceId"),
    ...
    traces = [_obs_to_trace(o) for o in raw_obs]
