...
def fetch_kb_answer_traces(limit: int = 20, timeout: int = 60) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        # Fetch traces using legacy GET /api/public/traces (page-based pagination)
        ...
        while len(all_traces) < limit:
            params = urllib.parse.urlencode({
                "name": "kb_answer",
                "limit": min(100, limit - len(all_traces)),
                "page": page,
            })
            ...
            if len(batch) < min(100, limit - len(all_traces) + len(batch)):
                break
            page += 1

        print(f"Fetched {len(all_traces)} kb_answer traces", file=sys.stderr)
        return all_traces
