...
def fetch_all_traces(days, page_timeout=90, max_retries=4):
    ...
    page_num = 1
    while True:
        p = {"name": "kb_answer", "limit": 100, "fromTimestamp": from_date, "page": page_num}
        ...
        if body is None:
            ...
            break
        batch = [_trace_to_shape(t) for t in body.get("data", [])]
        all_traces.extend(batch)
        print(f"  fetched page {page_num}: {len(batch)} traces ({len(all_traces)} total)")
        if len(batch) < 100:
            break
        page_num += 1
...
def main():
    ...
    for t in candidates:
        ...
        trace_id = t["id"]
        body_payload = {"metadata": event["batch"][0]["body"]["metadata"],
                        "userId": event["batch"][0]["body"]["userId"]}
        endpoint = f"{host}/api/public/traces/{trace_id}"
        req = urllib.request.Request(endpoint, data=json.dumps(body_payload).encode(), headers=headers, method="PUT")
        try:
