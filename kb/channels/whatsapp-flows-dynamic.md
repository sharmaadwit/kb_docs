> Source: https://console-docs.gupshup.io/docs/whatsapp-flows-enhancement-dynamic-flow-support
...
<Image align="center" src="https://files.readme.io/41a62dd5d808b43d9c47aaa46a0bbc5169c7252c81f866bdd83a9513eb211dff-WA_Dynamin.png" />
...
1. On WhatsApp Flow node click on the First Screen Dropdown to view the options
2. Select the type of First Screen that will be sent
3. Incase of Dynamic Flow select **Data Exchange** and for Static Flow select **Navigate**
4. Continue configuring the node as required and the Flow will be triggered with the same config during runtime
5. For Dynamic Flows make sure you have the Data Channel mapping done on the Meta BM while creating the Flow JSON
6. To receive the responses submitted on the WA Flows on Console/WhatsApp Flow Journey, the Dynamic Channel have to send back the payload on the nfm reply at the Terminal Screen to use the same on the WhatsApp Flow Journey\\
...
For dynamic flows, establish a data channel that facilitates communication between the WhatsApp client and your server. This involves specifying the `endpoint_uri` when creating or updating your flow via the Flows API. Contact [Gupshup Support](mailto:console-support@gupshup.io) to update your endpoint for your WhatsApp Flows.
...
To know more on Implementing Endpoints for Dynamic Flow/Data Exchange: [Meta Doc](https://developers.facebook.com/docs/whatsapp/flows/guides/implementingyourflowendpoint)
...
For setting up the Data Channel for Dynamic Flows, refer to the [Meta Documentation](https://developers.facebook.com/docs/whatsapp/flows/guides/implementingyourflowendpoint) or reach out to Gupshup Support or Customer Success Team for hosting and pricing details.